...
local rep = mw.ustring.rep --shorter
local conf = {
	--COMMON IDS
...
local function createRow(label, rawValue, link, withUid, valid_id, pencil)
...
-- Creates a human-readable wikitable version of conf
function p.docConfTable(frame)
	local wikiTable = '\n{| class="wikitable sortable plainlinks" style="width:98%; margin:0"' ..
					  '\n|-' ..
	...
	for i = 1, #conf do
		local param = mw.ustring.lower(conf[i].id)
		local label = conf[i].label or conf[i].id
		local property = tonumber(conf[i].property)
		if property then
			property = '[[:d:Property:P' .. property .. '|P' .. property .. ']]'
		else
			property = '—'
		end
		local rx_validation = conf[i].pattern and #conf[i].pattern > 0 and 'да' or 'не'
		local valid_link = getLink(conf[i].property, '', label).text and 'да' or 'не'
		--concat
		wikiTable = wikiTable ..
					'\n|-' ..
					'\n| ' .. param ..
...
function p.authorityControl(frame)
	...
	local parentArgs = {}
	local fromTitleCount, rowCount = 2, 0
	local outString = ''
	...
	for f = 1, fromTitleCount do
		...
		if parentArgs['from' .. f] then
			--Fetch Wikidata item
			if parentArgs['from' .. f] ~= title then
				...
				for i = 1, #conf do
					...
					if not nilOrEmpty(val) then
						...
						if nilOrEmpty(pattern) then
							--non-existant pattern; assume the value is always valid
							validValue = true
	...
	--adjust navbox for number of rows and create
	if rowCount > 0 then
		tCats[1] = '' --AC is not empty
		if rowCount == 1 then
			navboxParams['group' .. rowCount] = 'Нормативен контрол' .. navboxParams['wd-edit' .. rowCount]
		else
