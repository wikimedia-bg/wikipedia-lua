...
function Config:getUnitSymbol(id, raw, link, short)
	...
	-- get label
	if not symbol then
		symbol, lang = mw.wikibase.getLabelWithLang(id)
	...
	elseif link then
		...
		if title then
			symbol = buildWikilink(title, symbol)
		end
	end
...
function Config:getValue(snak, raw, link, lat_only, lon_only, short, anyLang, unitOnly, noSpecial)
...
function State:getProperty(claim)
	local value = {self.conf:getValue(claim.mainsnak, self.rawValue, self.linked, self.lat_only, self.lon_only, self.shortName, self.anyLanguage, self.unitOnly)}  -- create one value object
...
function State:getQualifier(snak)
	local value = {self.conf:getValue(snak, self.rawValue, self.linked, self.lat_only, self.lon_only, self.shortName, self.anyLanguage, self.unitOnly)}  -- create one value object
...
function claimCommand(args, funcName)
	...
	if value == '' and _.tracking then
		...
		mw.addWarning(text)
		local preprocess = tonumber(mw.getCurrentFrame():preprocess('{{REVISIONID}}'))
		if not preprocess or preprocess == 0 then
			value = text
		end
	end
