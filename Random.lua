...
local makeList = require('Модул:List').makeList
...
local function raiseError(msg)
	...
	return mw.ustring.format('<b class="error">[[Модул:Random]] error: %s.</b>', msg)
end
...
function l.html_list(args, listType)
	-- Randomizes a list and turns it into an HTML list. Uses [[Модул:List]].
	listType = listType or 'bulleted'
	local listArgs = makeRandomList(args) -- Arguments for [[Модул:List]].
